...
InitPage(page);
connect(this,SIGNAL(clicked(QIconViewItem*)),this,SLOT(setSymbolCode(QIconViewItem*)));
}
...
switch (page)
 {
  case 1:
    ...
    for ( uint i = 0; i <= 225; i++ )
     ...
     if ( pixmap.load(locate("data","kile/mathsymbols/"+icon_name)) )
        {
...
void SymbolView::setSymbolCode(QIconViewItem* item)
{
if (item)
   {
   codesymbol=item->key();
   emit SymbolSelected(codesymbol);
   }
else codesymbol="";
}
