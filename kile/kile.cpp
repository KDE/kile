...
QString Kile::prepareForCompile(const QString & command) {
  ...
  {
	finame=getName();
	if (currentEditor() && !currentEditor()->isLaTeXRoot())
	{
...
if (singlemode)
{
	path = QFileInfo(getName()).dirPath();
}
else
{
	path = QFileInfo(MasterName).dirPath();
}
...
if (s=="include")
    {
    QString fname=*it1;
    if (fname.right(5)==".tex}") fname=path+"/"+fname.mid(1,fname.length()-2);
    else fname=path+"/"+fname.mid(1,fname.length()-2)+".tex";
    QFileInfo fi(fname);
...
void Kile::LatexHelp()
{
    ...
    if (fic.exists() && fic.isReadable() )
      ...
      partManager->addPart(htmlpart, true);
      partManager->setActivePart( htmlpart);
      htmlpart->openURL(KURL::fromPathOrURL(fic.absFilePath()));
