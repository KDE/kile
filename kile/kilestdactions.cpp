...
void setupStdTags(KileInfo *ki, KMainWindow *parent, QPtrList<KAction>* plist )
{
	...
	alist.append(new KileAction::InputTag(ki,"cite",0, parent, SLOT(insertTag(const KileAction::TagData&)), parent->actionCollection(),"tag_cite", parent, KileAction::KeepHistory, "\\cite{%R","}", 6,0, i18n("This command generates an in-text citation to the reference associated with the ref entry in the bib file\nYou can open the bib file with Kile to see all the available references"), i18n("Reference")));
