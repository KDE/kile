	...
	: KDialogBase( parent, name, true, i18n("Configure user help"), Cancel | Ok, Ok, true )
{
	...
	QVGroupBox* group= new QVGroupBox(i18n("Userhelp"),page );
...
void UserHelpDialog::updateButton()
{
	...
	if ( entries == 1 ) {
		...
		if ( index == 0 ) {                  // index = 0
			down_state = true;
		} else if ( index == entries-1 ) {   // index = entries-1
			sep_state = true;
			up_state = true;
	  ...
	  m_menulistbox(menulistbox)
{
	...
	QWidget *widget = new QWidget(group);
	QGridLayout *grid = new QGridLayout( widget, 2,4, 5,5, "" );
	grid->setColSpacing(2,12);
   ...
   m_pbChooseFile->setFixedWidth(m_pbChooseFile->sizeHint().width());     
...
void UserHelpAddDialog::slotChooseFile()
{
	...
	QString filename = KFileDialog::getOpenFileName( directory,filter,this,i18n("Select a file") );
	if ( filename.isEmpty() )
		return;
